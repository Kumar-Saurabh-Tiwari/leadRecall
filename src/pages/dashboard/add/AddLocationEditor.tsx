...
export default function AddLocationEditor() {
  ...
  const state = (location.state || {}) as {
    mediaUrl?: string;
    ocrText?: string;
    selectedEvent?: { eventId: string; eventName: string };
  };
  ...
  const handleAddressSelect = (
    ...
    if (coordinates) setLocationCoords(coordinates);
  };
    ...
    setTimeout(async () => {
      try {
        ...
        const data: any = {
          ...
          oLocationData: locationDetails,
          oProfileAnalyzeData: {},
          eMediaType: 'dataset',
          eRecordType: 'location',
          dCreatedDate: new Date().toISOString(),
          isOfflineRecord: false,
        };

        // Call backend API (same endpoints as contact/content flow)
        let apiResponse;
        ...
        entryService.add({
          name: label.trim() || locationAddress.trim() || 'Untitled location',
          company: '',
        ...
        toast({ title: 'Location saved', description: 'Your location was saved.' });
        refreshEntries();
        navigate('/dashboard');
      } catch (error) {
    ...
    <div className="p-4 min-h-full">
      ...
      <Card className="shadow-card border-border/50">
        ...
        <CardContent className="p-4 space-y-4">
          ...
          {/* Location with Google Maps suggestions */}
          <LocationSelector
          ...
          {/* Notes (pre-filled with scanned data) */}
          <Textarea
            placeholder="Notes"
            value={notes}
